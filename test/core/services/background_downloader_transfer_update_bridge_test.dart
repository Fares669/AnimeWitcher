...
void main() {
  test('transport exposes one global stream for Transfer API updates', () {
    ...
    expect(source, contains('final StreamController<TaskUpdate> _updates ='));
    expect(
      source,
      contains('Stream<TaskUpdate> get updates => _updates.stream;'),
    );

    final attachStart = source.indexOf('void _attach(Transfer transfer) {');
    final attachEnd = source.indexOf(
  ...
  test(
    'service bridges Transfer updates before plugin start and rehydrate',
    () {
