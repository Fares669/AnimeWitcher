...
void main() {
  test('plugin resume delegates fallback semantics to Transfer.resume', () {
    ...
    final start = source.indexOf('Future<bool> resume(DownloadTask task) async');
  ...
  test('plugin fresh start delegates duplicate lookup to Transfers.getOrStart', () {
    final source = File(
      'lib/core/services/background_downloader_transport.dart',
    ).readAsStringSync();
    final start = source.indexOf('Future<bool> start(DownloadTask task) async');
    final end = source.indexOf('Future<bool> pause(DownloadTask task) async', start);
    expect(start, greaterThanOrEqualTo(0));
    expect(end, greaterThan(start));
    final body = source.substring(start, end);
