...
void main() {
  test('native cancel keeps tracking until independent ownership proof', () {
    ...
    final start = source.indexOf('Future<DownloadCancelSettlement> cancel(');
    final end = source.indexOf('\n  void forget(', start);
    expect(start, greaterThanOrEqualTo(0));
    expect(end, greaterThan(start));
    final body = source.substring(start, end);
    expect(body, isNot(contains('_detach(task.taskId)')));
    expect(body, contains('DownloadCancelSettlement.stillOwned'));
    expect(body, contains('DownloadCancelSettlement.unknown'));
  });
