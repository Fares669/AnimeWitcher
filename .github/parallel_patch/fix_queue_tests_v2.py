...
def rewrite_test(title: str, replacements: list[tuple[str, str, int]], new_title: str | None = None) -> None:
    ...
    for old, new, expected in replacements:
        ...
        if count != expected:
            raise RuntimeError(f'{title}: expected {expected}, found {count}: {old[:70]!r}')
...
rewrite_test(
    ...
    [
        ...
        ("expect(afterEp1Finishes.idsToPromote, isEmpty);", "expect(afterEp1Finishes.idsToPromote, ['ep2']);", 1),
        ("""            live: const [
              LiveNativeDownload(
                taskId: 'ep2',
        ...
        expect(shouldStartDownloadLiveActivity(TaskStatus.running), isTrue);""", """          isFalse,
        );
        expect(shouldStartDownloadLiveActivity(TaskStatus.running), isTrue);""", 1),
    ],
