...
**2026-09-19 process-recreation + one-connection device regressions:** device log `download_v2(9).jsonl` proved two distinct root causes. First, 16-connection iOS transport was publishing live URLSession temp-file bytes as parent progress even though those bytes were not recoverable after process termination; the same generation fell from ~4.90% through ~4.07%/~2.14%/~0.10% before climbing again. V2 now keeps live/throughput telemetry separate from durable recovery progress, reports only completed/visible immutable-range bytes as resumable progress, and uses bounded ~1 MiB checkpoint ranges (up to 512 queued work units while the native writer ceiling remains 1-16). Second, a user-selected single iOS connection previously used one giant package DownloadTask; `background_downloader` pause delegates to iOS `cancelByProducingResumeData`, which can return no resume data and fail the task. Range-capable single-connection downloads now use the same durable immutable-range coordinator with exactly one active package DownloadTask writer, so Pause/Resume no longer depends on one giant resume blob. Range-unsupported origins still fail closed to the package single-task path rather than silently restarting from byte zero. Regression test `paused projection keeps progress-derived presentation bytes` was proven RED on run `35446066771` (expected 25, actual 0), then fixed. Flutter Checks run `35446226586` at app-code head `9e139e5767badcd483226f9b0f4d0a4069639900`: analyzer ✅, focused V2 **128/128** ✅, repository suite **1585 passed / 1 skipped** ✅, iOS no-codesign build/log ✅, native logger typecheck ✅. Physical-device confirmation remains required for the unchecked acceptance item.**
